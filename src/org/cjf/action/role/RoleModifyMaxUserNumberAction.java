...
public class RoleModifyMaxUserNumberAction extends AbstractWebAction<Role> {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(RoleModifyMaxUserNumberAction.class);
	
	public RoleModifyMaxUserNumberAction() {
	...
	protected boolean process(Object obj) {
		...
		int count = biz.modifyCustom(item, "updateMaxUserNumber");
		if(count != 1) {
			message = "修改失败，修改个数为0.";
	...
	protected boolean checkParam(Object obj) {
		...
		if(item.getMaxUserNumber() <= 0) {
			message = "MaxUserNumber <= 0";
			return false;
		}
